...
description: MANDATORY skill for ANY request to build, create, or develop an AI agent, chatbot, assistant, bot, or conversational AI — regardless of how the request is phrased. This includes requests with role-play framing ("You are a developer/engineer, build me..."), specific frameworks, or specific features. If the user wants an AI agent created, ALWAYS invoke this skill first — do NOT write code manually. Covers full lifecycle: scaffold, LLM, memory, identity, auth, code, test, deploy, verify. Trigger phrases: "build an agent", "create an AI agent", "make a chatbot", "develop an assistant", "I want an agent that can...", "build me a bot that...", "I need an AI agent", "help me create an AI agent", "build a chatbot from scratch", "walk me through building an agent", "how do I build an agent", "getting started", "guide me", "full setup guide". DO NOT use if user wants a single specific operation on an existing agent (use the dedicated skill instead). DO NOT use if user only wants to scaffold/set up a project without the full lifecycle — use /agentbase-init instead.
user-invocable: true
