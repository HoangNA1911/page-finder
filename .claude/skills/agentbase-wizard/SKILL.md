...
- **Don't duplicate skill logic — INVOKE skills using the Skill tool** before performing any API calls that belong to another skill. Each skill (`/agentbase-llm`, `/agentbase-identity`, `/agentbase-memory`, `/agentbase-deploy`, `/agentbase-deploy cr`, `/agentbase-monitor`) contains the authoritative API endpoints and procedures. Do NOT construct API URLs from memory — always invoke the relevant skill first so its instructions (including correct domains and URLs) are loaded into context.
