...
- `document_updates` is a changelog: `sync_pages` appends one row when a page is newly indexed or its version changes (not for forced/schema-only reindexes). `user_activity` holds each user's `last_seen_at` (general activity, advanced every request) and `last_update_check_at` (advanced only when the user checks for updates).

**"What's new?" flow** — `check_document_updates` is a **pure local lookup**: it reads the `document_updates` changelog filtered by the user's *previous* `last_update_check_at` (snapshotted in `runtime.handler` before the request, passed via the `last_update_check_before` configurable). It does **not** sync or re-fetch Confluence (no `ensure_index_ready`) — keeping the changelog current is the background sync job's job — so it adds no Confluence latency. The system prompt also forbids the agent from triggering a sync on these questions. `check_document_updates_impl` advances `last_update_check_at` to now whenever it runs (only on actual checks, **not** on every request), so the tool reports everything changed since the user's *prior update check*. Brand-new users (no `last_update_check_at`) get a "first time checking" message instead of a flood.
